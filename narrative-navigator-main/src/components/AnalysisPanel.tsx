...
export function AnalysisPanel({ isAnalyzed, overallScore, editLog }: AnalysisPanelProps) {
  ...
  return (
    ...
    >
      ...
      {/* Explainable Output: Original | Modified | Reason */}
      <Section icon={<FileSearch className="w-4 h-4" />} title="Explainable Output">
        {editLog.length === 0 ? (
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-left py-2 px-2 font-medium text-muted-foreground">Original</th>
                  <th className="text-left py-2 px-2 font-medium text-muted-foreground">Modified</th>
              ...
              <tbody>
                {editLog.map((e, i) => (
                  ...
                  >
                    <td className="py-2 px-2 text-destructive/90 line-through align-top break-words">
                      {e.original || "—"}
                    </td>
                    <td className="py-2 px-2 text-accent font-medium align-top break-words">
                      {e.modified || "—"}
                    </td>
